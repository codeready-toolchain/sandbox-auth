...
import (
	...
	repository2 "github.com/codeready-toolchain/sandbox-auth/pkg/application/repository"
...
type serviceContextImpl struct {
	...
	transactionalRepositories repository2.Repositories
...
func (s *serviceContextImpl) Repositories() repository2.Repositories {
	if s.inTransaction {
...
func (s *serviceContextImpl) ExecuteInTransaction(todo func() error) error {
	if !s.inTransaction {
		...
		s.transactionalRepositories = tx.(repository2.Repositories)

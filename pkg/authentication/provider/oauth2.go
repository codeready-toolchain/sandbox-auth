...
func NewIdentityProvider(config IdentityProviderConfiguration) *DefaultIdentityProvider {
	...
	provider.Scopes = strings.Fields(config.GetOAuthProviderScopes())
...
func (provider *DefaultIdentityProvider) Profile(ctx context.Context, token oauth2.Token) (*UserProfile, error) {
	...
	log.Info(ctx, map[string]interface{}{
		"body": body,
	}, "Unmarshalling user profile payload")
	...
	if err != nil {
		log.Error(ctx, map[string]interface{}{
			"err":  err,
			"json": body,
		}, "error unmarshalling userinfo", err)
		return nil, err
	}
	...
	if idpResponse.Username != "" {
		u.Username = idpResponse.Username
	} else if idpResponse.PreferredUsername != "" {
		u.Username = idpResponse.PreferredUsername
	}
...
func (provider *DefaultIdentityProvider) UserProfilePayload(ctx context.Context, token oauth2.Token) ([]byte, error) {
	...
	if err != nil {
		log.Error(ctx, map[string]interface{}{
			"err":         err.Error(),
			"profile_url": provider.ProfileURL,
		}, "unable to retrieve user profile from provider")
		return nil, err
	}
	...
	if res.StatusCode < 200 || res.StatusCode > 299 {
		...
		for k := range res.Header {
			headerValues = append(headerValues, fmt.Sprintf("%s:%v,", k, res.Header.Values(k)))
		}
